...
def get_goals(data, topic, num_tokens, temperature):
    response = openai.Completion.create(
        engine="text-davinci-002",
        prompt=data + "\n\n\n" +
               "1. What are the goals of " + topic + "?\n" +
        "Answer the topic questions for " + topic +":\n\n\n", 
...
def get_key_concepts(data, topic, num_tokens, temperature):
    response = openai.Completion.create(
        engine="text-davinci-002",
        prompt=data + "\n\n\n" +
     "What are the key concepts " + topic + "?\n" +
        "Answer the topic questions for " + topic +":\n\n\n",
        temperature=temperature,
        max_tokens=num_tokens,
        top_p=1.0,
...
def main():
    topic = input("Topic: ")
    data, num_tokens = extract_topic_data(topic, "", 900, 2)
    ...
    print("Data: " + data + "\n")
    print("test")
    print(get_goals(data, topic, num_tokens, 0.4).choices[0].text)
    print(get_key_concepts(data, topic, num_tokens, 0.4).choices[0].text)
